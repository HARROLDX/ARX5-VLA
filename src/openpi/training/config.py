...
import openpi.model_dp_left.configuration_dp_left as dp_left_config
...
import openpi.policies.arx_left_policy as arx_left_policy
...
@dataclasses.dataclass(frozen=True)
class LerobotARXLeftDataConfig(DataConfigFactory):
    """Data config for the left-arm-only ARX LeRobot dataset."""
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        if self.include_third_camera:
            original_repack = repack_transforms.inputs[0]
            new_structure = dict(original_repack.structure)
            new_images = dict(new_structure["images"])
            new_images["camera_third"] = "observation.images.camera_third"
            new_structure["images"] = new_images
            repack_transforms = _transforms.Group(inputs=[_transforms.RepackTransform(new_structure)])

        data_transforms = _transforms.Group(
            inputs=[
                arx_left_policy.ARXLeftInputs(
                    action_dim=model_config.action_dim,
                    model_type=model_config.model_type,
                    mask_state=self.mask_state,
                    include_third_camera=self.include_third_camera,
                )
            ],
            outputs=[arx_left_policy.ARXLeftOutputs()],
        )
...
class TrainConfig:
    ...
    seed: int = 42
    # 随机种子。控制权重初始化、数据shuffle、训练噪声等所有随机行为
    # 同一个seed → 同样的训练结果（可复现）
    # 一般不用改，除非你想跑多次对比实验用不同seed

    batch_size: int = 32
    # 每一步训练用多少条数据算一次梯度
    # 256 = 每步从数据集取256帧（每帧含3张图+state+action）一起算loss再更新权重
    # 越大训练越稳定但越吃显存，必须能被GPU数整除

    num_workers: int = 2
    # data loader 的并行读取进程数
    # 8 = 开8个子进程同时从parquet读数据、从MP4解码视频帧
    # 越大数据加载越快，但越吃CPU和内存
    # 如果CPU核心少或内存紧张就调小

    num_train_steps: int = 30_000
    # 总共训练多少步（不是epoch）
    # 每步处理一个batch，所以总共看到的数据量 = num_train_steps × batch_size
    # 100_000步 × 256 batch = 2560万帧
    # 注意：数据集会无限循环采样，不存在"跑完一遍"的概念

    log_interval: int = 100
    # 每100步打印一次训练指标（loss、grad_norm、param_norm）
    # 同时写入wandb
    # 调小可以看到更细的loss曲线，但会略微拖慢训练
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        data=LerobotARXDataConfig(
            repo_id="/mnt/workspace/xiajiawei/kai0/data/task_1_collect_001_lerobot",
            assets=AssetsConfig(
                assets_dir="/mnt/workspace/xiajiawei/kai0/data",
                asset_id="task_1_collect_001_lerobot",
            ),
            default_prompt="Fetch and hang the cloth.",
            use_delta_joint_actions=False,
        ),
    ...
    TrainConfig(
        name="dp_arx_left",
        model=dp_left_config.DPLeftConfig(
            action_dim=7,
            action_horizon=32,
            n_obs_steps=1,
            n_action_steps=32,
        ...
        data=LerobotARXLeftDataConfig(
            repo_id="/mnt/workspace/xiajiawei/kai0/data/task_1_left_single_collect_001_lerobot",
            assets=AssetsConfig(
                assets_dir="/mnt/workspace/xiajiawei/kai0/data",
                asset_id="task_1_left_single_collect_001_lerobot",
            ),
            default_prompt="grasp the cup and place it on the black plate",
            use_delta_joint_actions=False,
        ),
        num_train_steps=20_000,
        keep_period=5000,
    ...
    TrainConfig(
        ...
        data=LerobotARXLeftDataConfig(
            repo_id="/mnt/workspace/xiajiawei/kai0/data/task_1_left_single_collect_dp-1_lerobot_3cam",
            assets=AssetsConfig(
                assets_dir="/mnt/workspace/xiajiawei/kai0/data",
                asset_id="task_1_left_single_collect_dp-1_lerobot_3cam",
            ),
            default_prompt="grasp the cup and place it on the black plate",
            use_delta_joint_actions=False,
...
# 过拟合的dp测试版本
    TrainConfig(
        ...
        data=LerobotARXDataConfig(
            repo_id="/mnt/workspace/xiajiawei/kai0/data/task_1_collect_001_ep24_lerobot",
            assets=AssetsConfig(
                assets_dir="/mnt/workspace/xiajiawei/kai0/data",
                asset_id="task_1_collect_001_ep24_lerobot",
            ),
            default_prompt="grasp the cup and place it on the black plate",
            use_delta_joint_actions=False,
        ),
    ...
    TrainConfig(
        ...
        data = LerobotAgilexDataConfig(
            ...
            default_prompt="Flatten and fold the cloth.",
            use_delta_joint_actions=False,
            base_config=DataConfig(prompt_from_task=True),# prompt覆盖上述默认
        ),
