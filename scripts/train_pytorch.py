...
import openpi.model_dp.configuration_dp
...
def train_loop(config: _config.TrainConfig):
    ...
    # Build model
    if isinstance(config.model, openpi.model_dp.configuration_dp.DPConfig):
        model_cfg = config.model
        model = config.model.create_pytorch().to(device)
        logging.info("Training mode: ARX diffusion policy")
    else:
    ...
    if is_main:
        ...
        logging.info(f"Training precision: {getattr(model_cfg, 'dtype', config.pytorch_training_precision)}")
